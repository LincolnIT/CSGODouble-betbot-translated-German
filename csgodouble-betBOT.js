// ==UserScript==
// @name            csgodouble.com - automated by Lincoln
// @description     		Ein Skript für CSGODOUBLE.COM. Das Skript Wettet automatisch in einem ausgesuchten System. Bis zu 25 % Profit täglich !
// @namespace       automated@Lincoln
// @version         1.41 translated to German
// @author          Lincoln
...
function Automated() {
    ...
    menu.innerHTML = '' +
        '<div class="row">' +
            '<div class="col-lg-9">' +
                '<h2>CSGODouble.com - BetBOT <small>by Lincoln</small> <i id="automated-theme-switch" class="fa fa-lightbulb-o" style="cursor: pointer;"></i></h2>' +
                '<div class="form-group">' +
                    '<div class="btn-group">' +
                        ...
                        '<button type="button" class="btn btn-danger" id="automated-abort" disabled>Stoppe</button>' +
                    '</div>' +
                '</div>' +
                '<div class="form-group">' +
                    '<div class="btn-group">' +
                        '<button type="button" class="btn btn-default" id="automated-martingale" ' + (this.method === 'martingale' ? 'disabled' : '') + '>Martingale (x2)</button>' +
                        '<button type="button" class="btn btn-default" id="automated-great-martingale" ' + (this.method === 'great martingale' ? 'disabled' : '') + '>Great Martingale (x2+1)</button>' +
                        '<button type="button" class="btn btn-default" id="automated-dalembert" ' + (this.method === 'dalembert' ? 'disabled' : '') + '>D\'alembert</button>' +
                        '<button type="button" class="btn btn-default" id="automated-bet-green" ' + (this.method === 'green' ? 'disabled' : '') + '>Jagd auf grün (Fibonacci)</button>' +
                    '</div>' +
                '</div>' +
                '<div class="form-group automated-hide-on-green">' +
                    '<div class="btn-group">' +
                        '<button type="button" class="btn btn-default" id="automated-red" ' + (this.color === 'red' ? 'disabled' : '') + '>Rot</button>' +
                        '<button type="button" class="btn btn-default" id="automated-black" ' + (this.color === 'black' ? 'disabled' : '') + '>Schwarz</button>' +
                        '<button type="button" class="btn btn-default" id="automated-rainbow" ' + (this.color === 'rainbow' ? 'disabled' : '') + '>Regenbogen</button>' +
                        '<button type="button" class="btn btn-default" id="automated-random" ' + (this.color === 'random' ? 'disabled' : '') + '>Zufällige Farbe</button>' +
                        '<button type="button" class="btn btn-default" id="automated-last" ' + (this.color === 'last' ? 'disabled' : '') + '>Letzte Farbe</button>' +
                    '</div>' +
                '</div>' +
            '</div>' +
            '<div class="col-lg-3">' +
                '<h3>Statistiken</h3>' +
                '<p><b>Gewonnen:</b> <span id="automated-stats-wins">' + this.stats.wins + '</span></p>' +
                '<p><b>Verloren:</b> <span id="automated-stats-loses">' + this.stats.loses + '</span></p>' +
                '<p><b>Balance:</b> <span id="automated-stats-balance">' + this.stats.balance + '</span></p>' +
            '</div>' +
        '</div>' +
        '<div class="form-group">' +
            '<div class="input-group">' +
                '<div class="input-group-addon">Grundwert Wette</div>' +
                '<input type="number" class="form-control" placeholder="Obliczanie sugerowanej wartości..." id="automated-base-bet" disabled>' +
            '</div>' +
        '</div>' +
        '<div class="form-group">' +
            '<div class="input-group">' +
                '<div class="input-group-addon">Behalte den letzten Wert</div>' +
                '<input type="number" class="form-control" value="0" id="automated-min-balance">' +
            '</div>' +
        '</div>' +
        '<div class="form-group automated-hide-on-green">' +
            '<div class="input-group">' +
                '<div class="input-group-addon">Wert bei dem die Wette gestoppt werden soll</div>' +
        ...
        '</div>' +
        '<div class="checkbox">' +
            '<label><input class="" id="automated-stop-on-min-balance" type="checkbox" ' + (this.stop_on_min_balance ? 'checked' : '') + '>Stoppe bei Minimalwert ( Wenn das Skript nah genug an dem Minimalwert ist, sonst wird das Skript weiter von der Basis wetten)</label>' +
        '</div>' +
        ...
        '</div>' +
        '<div class="checkbox">' +
            '<label class="text-muted"><input id="automated-simulation" type="checkbox" ' + (this.simulation ? 'checked' : '') + ' disabled> Symulacja</label>' +
        '</div>';
